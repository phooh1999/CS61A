...
def floor_div(args):
    ...
    if args.rest == nil:
        ...
        res = Pair(args.first // args.rest.first, args.rest.rest)
        return floor_div(res)
...
def eval_and(expressions):
    ...
    True
    >>> calc_eval(Pair("and", Pair(Pair("and", Pair(False, Pair("1", nil))), Pair(1, nil))))
    False
    """
    if expressions == nil:
        return scheme_t
...
def eval_define(expressions):
    ...
    val = expressions.rest.map(calc_eval)
    bindings[var] = val.first
    return var
