...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(symbol, do_lambda_form(new_expr , env))
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    check_val = scheme_eval(expressions.first, env)
    if is_scheme_false(check_val):
        return False
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        value = scheme_eval(check.rest.first, env)
