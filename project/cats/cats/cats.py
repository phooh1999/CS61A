...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def helper(s):
        paragraph = split(lower(remove_punctuation(s)))
...
def accuracy(typed, source):
    ...
    correct = 0
    if typed_total == 0 and source_total == 0:
        return 100.0
    elif (typed_total and source_total) == 0:
        return 0.0
    else:
        for i in range(0, min(typed_total, source_total)):
            if typed_words[i] == source_words[i]:
                correct += 1
        return 100 * correct / typed_total
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    typed_time = elapsed / 60
    return typed_words / typed_time
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    diff_word = min(word_list, key=helper)
    if helper(diff_word) > limit:
        return typed_word
    else:
        return diff_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if len(typed) == 0 and len(source) == 0:
        return 0
    elif (len(typed) and len(source)) == 0:
...
def final_diff(typed, source, limit):
    ...
    if typed[0] == source[0]: # Feel free to remove or add additional cases
        return minimum_mewtations(typed[1:], source[1:], limit)
    elif (len(typed) > 1 and len(source) > 1) and (typed[0] == source[1] or typed[1] == source[0]):
...
def time_per_word(words, timestamps_per_player):
    ...
    for timestamp in timestamps_per_player:
        ...
        for i in range(0, len(timestamp)-1):
            time.append(timestamp[i+1] - timestamp[i])
        times.append(time)
...
def fastest_words(match):
    ...
    for word_i in word_indices:
        time_i = []
        for player_j in player_indices:
            time_i.append(time(match, player_j, word_i))
        fast_i = time_i.index(min(time_i))
        res_words[fast_i].append(get_word(match, word_i))
    return res_words
    # END PROBLEM 10
