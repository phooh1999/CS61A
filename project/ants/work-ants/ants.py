...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        check_place, dis = self.place, 0
        while not check_place.is_hive:
            if len(check_place.bees) != 0 and self.lower_bound <= dis and dis <= self.upper_bound:
                return random_bee(check_place.bees)
            else:
                check_place, dis = check_place.entrance, dis + 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for hit_bee in bees_list.copy():
            hit_bee.reduce_health(reflected_damage)
        # END Problem 5
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        if len(self.place.bees) != 0:
            for hit_bee in self.place.bees.copy():
                hit_bee.reduce_health(self.damage)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while check_place:
            check_ant = check_place.ant
            if check_ant:
                check_ant.double()
                if check_ant.is_container and check_ant.ant_contained:
                    check_ant.ant_contained.double()
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        check_place, dis = self.place, 0
        while not check_place.is_hive:
            bees_list = check_place.bees
            ant = check_place.ant
            for hit_bee in bees_list:
                res[hit_bee] = dis
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        hit_damage = self.damage - distance * 0.25 - self.insects_shot * 0.0625
        if hit_damage <= 0:
            return 0
        else:
            return hit_damage
