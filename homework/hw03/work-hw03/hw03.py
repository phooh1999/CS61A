...
def interleaved_sum(n, odd_func, even_func):
    ...
    def helper(k,n):
        if k == n:
            ...
            return odd_func(k) + even_func(k + 1) + helper(k + 2, n)
    return helper(1, n)
...
def count_coins(total):
    ...
    def helper(val, total):
        ...
        if total < 0:
            ...
            return helper(val, total - val) + helper(next_larger_coin(val), total)
...
def make_anonymous_factorial():
    ...
    return (lambda f: f(f))(lambda f: lambda x: x if x == 1 else x*f(f)(x-1))
